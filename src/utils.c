...
void parse_args(int argc, char **argv, SimConfig *cfg){
    ...
    for (int i = 1; i < argc - 1; i++) {
        if (strcmp(argv[i], "--threads") == 0) {
            int v = atoi(argv[i + 1]);
            if (v < MIN_WORKERS || v > MAX_WORKERS) // warning printed to terminal
                fprintf(stderr, "[WARN] --threads %d out of range [%d,%d], clamped to %d\n", v, MIN_WORKERS, MAX_WORKERS, (v < MIN_WORKERS) ? MIN_WORKERS : MAX_WORKERS);
            cfg->num_threads = (v < MIN_WORKERS) ? MIN_WORKERS : (v > MAX_WORKERS) ? MAX_WORKERS : v;
            i++;
        } else if (strcmp(argv[i], "--clients") == 0) {
            int v = atoi(argv[i + 1]);
            cfg->num_clients = (v < 1) ? 1 : (v > MAX_CLIENTS) ? MAX_CLIENTS : v;
            i++;
...
const char *log_event_name(LogEventType e) {
    switch (e) {
        ...
        case LOG_MSG_DROPPED:    return "MSG_DROPPED";
        case LOG_BUFFER_RESUME:  return "BUFFER_RESUME"; 
        case LOG_SEM_BLOCK:      return "SEM_BLOCK";
