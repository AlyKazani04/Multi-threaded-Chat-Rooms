...
static void make_message(int room_id, int client_id, const char *name, char *out, size_t out_sz){
    const char *phrase;
    switch (room_id) {
...
void *client_generator_thread(void *arg){
    ...
    long long last_sample_us = g_sim.start_time_us;
    ...
    while (atomic_load(&g_sim.running)) {
        ...
        if (now - last_sample_us >= 1000000LL) {
            int current = atomic_load(&g_sim.total_messages);
            int idx = g_sim.throughput_sample_count;
            if (idx < 3600) {
                g_sim.throughput_samples[idx] = current - prev_msg_count;
                g_sim.throughput_sample_count++;
            }
        ...
        if (client_seq > 0 && client_seq % 8 == 0 && cfg->num_threads >= 2){
            ...
            snprintf(t.message, sizeof(t.message), "[DM to T%d] Hey T%d, private message from %s (#%d)", to_t, to_t, CLIENT_NAMES[name_idx], client_seq);

            printf("[%08.3f] [PM-GEN] Client-%d (%s, T%d) -> T%d (private)\n", (double)(now_us() - g_sim.start_time_us) / 1e6, client_seq, t.sender, from_t, to_t);
        }
        else {
            t.is_private = false;
            t.room_id    = room_id;
            make_message(room_id, client_seq, CLIENT_NAMES[name_idx], t.message, sizeof(t.message));
        }

        taskqueue_push(&t);

        if (!t.is_private) {
            printf("[%08.3f] Client-%d (%s) >> %s: '%s'\n", (double)(now_us() - g_sim.start_time_us) / 1e6, client_seq, t.sender, g_sim.rooms[room_id].name, t.message);
        }
