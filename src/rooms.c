...
void rooms_init(void){
    ...
    for (int r = 0; r < NUM_ROOMS; r++) {
        ...
        room->total_mutex_wait_ns = 0;
        room->mutex_wait_count    = 0;
    ...
    printf("[INFO] 3 chat rooms initialised: Room A (General), Room B (Priority), Room C (Private)\n");
}

// Starts one consumer_thread per room with SLOW consumption rate. Messages stay visible in GUI for ~25 seconds at 2 msg/sec drain rate.
void rooms_consumer_init(void){
    for (int r = 0; r < NUM_ROOMS; r++) {
        ...
        *room_idx = r;
        ...
        int rc = pthread_create(&g_sim.consumer_tids[r], NULL, consumer_thread, room_idx);
        if (rc != 0) {
            fprintf(stderr, "[FATAL] pthread_create failed for consumer room %d (rc=%d)\n", r, rc);
            exit(EXIT_FAILURE);
        }
    }
...
bool room_write(int room_id, const char *sender, const char *msg, int thread_id) {
    ...
    room->total_mutex_wait_ns += wait_ns;
    room->mutex_wait_count++;
    ...
    while (room->count >= BUFFER_SLOTS) {
        ...
        logger_log(thread_id, LOG_RELEASED_LOCK, room_char, -1, (int)strlen(msg), 0, sender);
    ...
    if (wrapped)
        logger_log(thread_id, LOG_BUFFER_WRAP, room_char, -1, (int)strlen(msg), 0, sender);
