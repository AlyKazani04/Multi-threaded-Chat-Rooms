 ...
 *
 * FIX SUMMARY:
 *   FIX-A/B  : consumer_thread per room + total_consumed field (deadlock fix)
 *   FIX-G    : TOCTOU race on thread state removed
 *   FIX-F/J  : total_dropped counter + LOG_MSG_DROPPED + LOG_BUFFER_RESUME
 *   FIX-I    : LOG_SEM_BLOCK distinguishes pre-wait from post-admit
 *   FIX-M/N  : Private messaging now truly randomised via rand_mutex
 *   FIX-Q    : rand_mutex added so all rand() calls are thread-safe
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
typedef enum {
    ...
    LOG_PRIVATE_MSG,
    LOG_RATE_LIMITED,
    LOG_MSG_DROPPED,     /* FIX-F: explicit drop event        */
    LOG_BUFFER_RESUME,   /* FIX-D: producer woke from cond    */
    LOG_SEM_BLOCK        /* FIX-I: about to block on sem_wait */
} LogEventType;
...
typedef struct {
    ...
    int           count;
    int           total_written;
    int           total_consumed;   /* FIX-A: consumer drain counter */
...
typedef struct {
    ...
    atomic_int    total_dropped;       /* FIX-F */
    ...
    pthread_mutex_t rand_mutex;   /* FIX-Q: protects rand() global state */
} SimState;
...
void  rooms_consumer_init(void);     /* FIX-A */
void  rooms_consumer_shutdown(void); /* FIX-S */
